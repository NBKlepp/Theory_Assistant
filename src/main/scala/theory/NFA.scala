...
class NFA() extends FiniteAutomaton
{
    ...
    //:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
      ...
      def this(states : Set[State], alphabet : Set[Symbl], delta : Map[(State,Symbl),Set[State]], start : State, accept : Set[State], name : String ){
      	  this()
    ...
    private def setAccept(accept : Set[State]) {
	       ...
	       this.accept ++= accept
	    	 ...
	    	 for ( state <- accept if ( !(this.states contains state) ) ) println(s"Invalid accept state for $name : $state")
		 valid = false
	    } // else
	...
	for ( state <- states ) {
		...
		if ( !(state.equals(start)) ){
		   if (  (accept contains state) && a.equals(epsilon) ) delta += input -> ( this.delta(input) ++ Set(this.start) )
	...
	for( state <- states ){
	     for ( symbol <- sigma + epsilon ) {
		    ...
		    if( symbol.equals(epsilon) ) delta += input -> (Set(m2.getStartState()) ++ mm.delta(input))
	...
	while ( check.size >0 ){								   // While there are states we have not yet found the transition for
		  ...
		  for ( a <- alphabet if !(a.equals(epsilon))){							   // For each character in the alphabet
		      sigma += a
		      if (DEBUG) println(s"checking transitions from setState: ${setState} on symbol: $a")
		      val newSetState = setState.flatMap( x => this.delta( x,a ) )
		      if (DEBUG) println(s"Found the following setState without epsilon transitions: ${newSetState}")
		      val epsilonTransitions = newSetState.flatMap(epsilonTransitionDFAify(_))
		      if (DEBUG) println(s"Found the follwoing setState using epsilon transitions: ${epsilonTransitions}")
		      newSetState ++= epsilonTransitions
		      if (DEBUG) println(s"The final setState transition for (${setState},$a): ${newSetState}")
		      delta1 += (setState,a) -> newSetState 
		      if (DEBUG) println(s"The delta1 now says: ${delta1}")
	...
	for ( key <- delta1.keys ) {									// Look at each key in the proto-delta in turn
	    if (DEBUG) println(s"Creating a new transition: ${key}")
	    val setState = key._1
	    if (DEBUG) println(s"Working on setState: ${setState}")	    
	    ...
	    if (DEBUG) println(s"Transition: ($fromState,$symbl)->$toState")
...
*/
	...
	if ( ((this.states - NullState) intersect (m.states - NullState) ).size != 0 ) {
	   if (DEBUG) println(s"${getName()}.states: ${this.getStates()}")
	   if (DEBUG) println(s"${m.getName()}states: ${m.getStates()}")
	   val newStates = Map[State,State]()
	   val states = Set[State]()
	   val delta = Map[(State,Symbl),Set[State]]()
	   for( state <- this.states if !(state.equals(NullState)) ){
	   	if (DEBUG) println(s"Uniqueifying state $state.")
	   	var newState = state
		while( m.getStates() contains newState) newState = newState + p
		if ( !(newState.equals(state)) ) while( this.states contains newState ) newState = newState + p 
		while ( states contains newState) newState = newState + p
		if (DEBUG) println(s"newState: $newState")
		newStates += ( state -> newState)
		if (DEBUG) println(s"newStates: $newStates")
		states += newState
	   }
	   for( state <- this.states if !(state.equals(NullState)) ){
		for ( symbl <- this.alphabet if !(this.delta(state,symbl).equals(Set(NullState))) ) {
		    val newState = newStates(state)
		    delta += (newState,symbl) -> this.delta(state,symbl).filterNot(_ == NullState).map(x => newStates(x))
		} // for
	   } // for
	   val start = newStates(this.start)
	   val accept = Set[State]()
	   for ( state <- getAcceptStates() ) accept += newStates(state)
	   if (DEBUG) println(s"New machine: \n\tstates: $states \n\tstart: $start \n\taccept: $accept \n\tdelta: $delta")
	...
	val toStates    = transition._2
        i match{
	  case 1 => {
	  	 println(s"Invalid transition: delta($fromState,$inputSymbl)->$toStates) :: $fromState not in the set of states for ${name}.")
		 valid = false
	       } // case 1
	  case 2 => {
	  ...
	  case 3 => {
	    	 println(s"Invalid transition: delta($fromState,$inputSymbl)->$toStates) :: $inputSymbl not in the alphabet for ${name}.")
		 valid = false
	       } // case 3
	  case 4 => {
...
object NFATester extends App{
       ...
       val delta1  = mapFromSet( Set((("0","0"),Set("1")),(("0","1"),Set("1")),(("1","0"),Set("0")),(("1","1"),Set("0"))) )
       val start1  = "0"
       val accept1 = Set("0")

       val states1DFA = Set("0","1")										//All even length strings
       val sigma1DFA  = Set("0","1")
       val delta1DFA  = mapFromSet2( Set((("0","0"),"1"),(("0","1"),"1"),(("1","0"),"0"),(("1","1"),"0")) )
       val start1DFA  = "0"
       val accept1DFA = Set("0")

       val states2 = Set("0","00","10","01","11")									//even 0s OR odd 1s
       val delta2  = mapFromSet( Set((("0",epsilon),Set("00","10")),
			 (("00","0"),Set("01")),(("00","1"),Set("00")),(("01","0"),Set("00")),(("01","1"),Set("01")),
			 (("10","0"),Set("10")),(("10","1"),Set("11")),(("11","0"),Set("11")),(("11","1"),Set("10"))))
       val start2  = "0"
       val accept2 = Set("00","11")

       val states2DFA = Set("00","01","10","11")										//Even 0's OR odd 1's
       val delta2DFA  = mapFromSet2( Set( (("00","0"),"10"),(("00","1"),"01"),(("10","0"),"00"),(("10","1"),"11"),
       	   	     	     (("11","1"),"10"),(("11","0"),"01"),(("01","1"),"00"),(("01","0"),"11")))
       val start2DFA  = "00"
       val accept2DFA = Set("00","01","11")

       val states2_1 = Set("00","01")											//even 0s 
       val delta2_1  = mapFromSet( Set((("00","0"),Set("01")),(("00","1"),Set("00")),(("01","0"),Set("00")),(("01","1"),Set("01"))))
       val start2_1  = "00"
       val accept2_1 = Set("00")

       val states2_2 = Set("10","11")											//odd 1s 
       val delta2_2  = mapFromSet( Set((("10","1"),Set("11")),(("10","0"),Set("10")),(("11","1"),Set("10")),(("11","0"),Set("11"))))
       val start2_2  = "10"
       val accept2_2 = Set("11")

       val states3 = Set("0","00","10","01","11")									//even 0s OR even 1s
       val delta3  = mapFromSet( Set((("0",epsilon),Set("00","10")),
			 (("00","0"),Set("01")),(("00","1"),Set("00")),(("01","0"),Set("00")),(("01","1"),Set("01")),
			 (("10","0"),Set("10")),(("10","1"),Set("11")),(("11","0"),Set("11")),(("11","1"),Set("10"))))
       val start3  = "0"
       ...
       val delta3DFA  = mapFromSet2( Set( (("00","0"),"10"),(("00","1"),"01"),(("10","0"),"00"),(("10","1"),"11"),
       	   	     	     (("11","1"),"10"),(("11","0"),"01"),(("01","1"),"00"),(("01","0"),"11")))
       val start3DFA  = "00"
       val accept3DFA = Set("00","01","10")

       val states4 = Set("0","00","10","01","11")									//odd 0s OR odd 1s
       val delta4  = mapFromSet( Set((("0",epsilon),Set("00","10")),
			 (("00","0"),Set("01")),(("00","1"),Set("00")),(("01","0"),Set("00")),(("01","1"),Set("01")),
			 (("10","0"),Set("10")),(("10","1"),Set("11")),(("11","0"),Set("11")),(("11","1"),Set("10"))))
       val start4  = "0"
       ...
       val delta4DFA  = mapFromSet2( Set( (("00","0"),"10"),(("00","1"),"01"),(("10","0"),"00"),(("10","1"),"11"),
       	   	     	     (("11","1"),"10"),(("11","0"),"01"),(("01","1"),"00"),(("01","0"),"11")))
       val start4DFA  = "00"
       val accept4DFA = Set("10","01","11")

       val states5 = Set("0","00","10","01","11")									//odd 0s OR even 1s
       val delta5  = mapFromSet( Set((("0",epsilon),Set("00","10")),
			 (("00","0"),Set("01")),(("00","1"),Set("00")),(("01","0"),Set("00")),(("01","1"),Set("01")),
			 (("10","0"),Set("10")),(("10","1"),Set("11")),(("11","0"),Set("11")),(("11","1"),Set("10"))))
       val start5  = "0"
       ...
       val delta5DFA  = mapFromSet2( Set( (("00","0"),"10"),(("00","1"),"01"),(("10","0"),"00"),(("10","1"),"11"),
       	   	     	     (("11","1"),"10"),(("11","0"),"01"),(("01","1"),"00"),(("01","0"),"11")  ))
       val start5DFA  = "00"
       val accept5DFA = Set("10","00","11")

       val states6 = Set("0","1","2","3","4")
       val delta6  = mapFromSet( Set((("0","0"),Set("0","1")),(("0","1"),Set("0")),
			 (("1","1"),Set("2")), 
			 (("2","1"),Set("3")),
			 (("3","0"),Set("4")),
			 (("4","0"),Set("4")),(("4","1"),Set("4"))))
       ...
       val delta6DFA  = mapFromSet2( Set((("0","1"),"0"),(("0","0"),"1"),
			    (("1","1"),"2"),(("1","0"),"1"),
			    (("2","1"),"3"),(("2","0"),"1"),
			    (("3","1"),"0"),(("3","0"),"4"),
			    (("4","1"),"5"),(("4","0"),"4"),
       ...
       val delta7  = mapFromSet( Set((("0","1"),Set("0","1")),(("0","0"),Set("0")),
       	   	         (("1","1"),Set("2"))))
       val start7  = "0"		 
       val accept7 = Set("2")

       val states7DFA = Set("0","1","2")
       val delta7DFA  = mapFromSet2( Set((("0","1"),"1"),(("0","0"),"0"),
			    (("1","1"),"2"),(("1","0"),"0"),
			    (("2","1"),"2"),(("2","0"),"0")))
       val start7DFA  = "0"		 
       ...
       val delta8  = mapFromSet( Set((("1","0"),Set("1","2")),(("1","1"),Set("1","4")),
			 (("2","0"),Set("3"))                             ,
			 (("3","0"),Set("3"))    ,(("3","1"),Set("3"))    ,
					          (("4","1"),Set("5"))    ,
			 (("5","0"),Set("5"))    ,(("5","1"),Set("5"))    ))
       ...
       val delta8DFA  = mapFromSet2( Set((("1","1"),"14"),(("1","0"),"12"),
			    (("12","1"),"14"),(("12","0"),"123"),
			    (("14","1"),"145"),(("14","0"),"12"),
			    (("123","1"),"143"),(("123","0"),"123"),
			    (("145","1"),"145"),(("145","0"),"125"),
       ...
       val statesS = Set("0","1")
       val deltaS  = mapFromSet( Set((("0","0"),Set("1")),(("0","1"),Set("1"))))
       val startS  = "0"
       val acceptS = Set("1")
       ...
       val deltaE0  = mapFromSet( Set((("0","0"),Set("1")),(("0","1"),Set("0")),
			  (("1","0"),Set("0")),(("1","1"),Set("1"))))
       val acceptE0 = Set("0")

       val deltaO0  = mapFromSet( Set((("0","0"),Set("1")),(("0","1"),Set("0")),
			  (("1","0"),Set("0")),(("1","1"),Set("1"))))
       val acceptO0 = Set("1")

       val deltaE1  = mapFromSet( Set((("0","0"),Set("0")),(("0","1"),Set("1")),
			  (("1","0"),Set("1")),(("1","1"),Set("0"))))
       val acceptE1 = Set("0")

       val deltaO1  = mapFromSet( Set((("0","0"),Set("0")),(("0","1"),Set("1")),
			  (("1","0"),Set("1")),(("1","1"),Set("0"))))
       val acceptO1 = Set("1")

       val states11 = Set("0","1","2")
       val start11  = "0"
       val delta11  = mapFromSet( Set((("0","1"),Set("1")),(("1","1"),Set("2"))))
       val accept11 = Set("2")

       val states00 = Set("0","1","2")
       val start00  = "0"
       val delta00  = mapFromSet( Set((("0","0"),Set("1")),(("1","0"),Set("2"))) )
       val accept00 = Set("2")
       ...
       val m16 = mS.kleeneStar().concat(m_11).concat(mS.kleeneStar()).union(mS.kleeneStar().concat(m_00).concat(mS.kleeneStar())) // equal to m8
